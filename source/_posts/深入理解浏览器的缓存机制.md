...
categories:
  ...
  - 浏览器
abbrlink: 29829
date: 2023-02-08 09:37:30
---

转载自：<https://www.jianshu.com/p/54cc04190252>
...
**如需获取思维导图或想阅读更多优质文章请猛戳[GitHub博客](https://github.com/ljianshu/Blog)**,也欢迎加我个人微信frontJS,一起交流技术！

{% asset_img map.png 浏览器缓存机制思维导图 %}
...
{% asset_img memory-cache.png 内存缓存 %}
...
Push Cache 在国内能够查到的资料很少，也是因为 HTTP/2 在国内不够普及。这里推荐阅读`Jake Archibald`的 [HTTP/2 push is tougher than I thought](https://jakearchibald.com/2017/h2-push-tougher-than-i-thought/) 这篇文章，文章中的几个结论：http-
...
{% asset_img http-request.png 第一次发起HTTP请求 %}
...
{% asset_img cache-control.png cache-control %}
...
{% asset_img min-fresh.png cache-control %}
...
- 协商缓存生效，返回304和Not Modified
  ...
  {% asset_img 协商缓存生效.png 协商缓存生效 %}

  协商缓存生效

- 协商缓存失效，返回200和请求结果

{% asset_img 协商缓存失效.png 协商缓存失效 %}
...
{% asset_img last-modify.png last-modify %}
...
{% asset_img etag.png ETag和If-None-Match %}
